...
class S3 {
  ...
  async putS3Object(body, bucket, objectKey) {
    ...
    console.log(`Uploading file: ${objectKey} to bucket: ${bucket}`);
    return new Promise((resolve, reject) => {
   ...
   */
  async putS3Stream(readStream, bucket, objectKey) {
    ...
    console.log(`Uploading File: ${objectKey} in multiple parts to bucket: ${bucket}`);
    ...
    try {
      ...
      while (chunk) {
        ...
        if (uploadPromises.length >= config.MAX_CONCURRENT) {
          ...
          console.group();
          console.group();
          console.log(`${objectKey}: Uploading batch #${batchCount}`);
          console.groupEnd();
          console.groupEnd();
          batchCount += 1;
      ...
      if (uploadPromises.length) {
        console.group();
        console.group();
        console.log(`${objectKey}: Uploading batch #${batchCount} (final)`);
        console.groupEnd();
        console.groupEnd();
        parts = parts.concat(await Promise.all(uploadPromises));
      }
